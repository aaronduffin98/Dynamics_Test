...
const SALES_STAFF_DETAIL_FIELD_IDS = SALES_STAFF_DETAIL_FIELDS.map((f) => f.id);
...
export default function SalesStaffGrid({
  ...
  const filteredItems = useMemo(() => {
    ...
    return salesStaff.filter((row) =>
      [row.salesStaffId, row.name, row.role, row.assignedDevelopmentName, row.assignedDevelopmentId, row.email].some((v) =>
      ...
      salesStaffId: {
        ...
        renderCell: (item) => (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              onOpenSalesStaff?.(item.salesStaffId);
            }}
            aria-label={`Open sales staff ${item.salesStaffId}`}
          >
            {item.salesStaffId}
          </button>
        ),
  ...
  const {
    ...
    detailFieldIds: SALES_STAFF_DETAIL_FIELD_IDS,
    columnDefs,
    sortState,
            ...
            <div className="dynamics-surface-card dynamics-surface-card--command">
              <div className="dynamics-commandbar" role="toolbar" aria-label="Commands">
                <div className="dynamics-commandbar__scroll">
                  ...
                  <Button appearance="subtle" type="button" onClick={() => onOpenNewSalesStaff?.()} title="Create new sales staff">
                    <span className="dynamics-cmd-btn__inner">
                  ...
                  <div ref={scrollRef} className="dynamics-grid-scroll dynamics-grid-scroll--list">
                    <div className="dynamics-grid-scroll__inner">
                      <DataGrid
                        ...
                        size="small"
                        getRowId={(item) => item.salesStaffId}
                        focusMode="composite"
